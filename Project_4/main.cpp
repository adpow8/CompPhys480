...
#include <random>
#include "time.h"
#include "galacticsystem.h"
#include "body.h"
#include "project4_lib.h"
#include "armadillo"
...
int main(int argc, char* argv[])
{
    ...
    int i, j, k, steps, system_choice, galaxy_size, method, outputtype, bodies;
    ...
    if(argc<8)
    {
        cout << "Bad usage. Enter also 'outfilename system_choice system_choice method_choice output_type steps tmax' on same line." << endl;
        cout << "Current options for system_choice include" << endl;
        cout << "\t1: 200 stars in a cold collapse" << endl << "\t2: coplanar earth-jupiter-sun system" << endl;
        cout << "\t3: full galactic system, with planets and luna" << endl;
    ...
    within our galactic system. Comment out any
    which are not desired for the calculation.
    Current options are
    1: 200 stars in cold collapse
    2:
    3:
    ********************************************/

    galacticsystem galaxy;

    if(system_choice==1){

        //random number generators
        default_random_engine generator;
        normal_distribution<double> mass_dis(10.0,1.0); //gaussian distibution
        uniform_real_distribution<double> pos_dis(0.0, 1.0);   //uniform distribution


        for (i=0; i<200; i++){
            double mass = mass_dis(generator), x = pos_dis(generator)*galaxy_size,
                    y = pos_dis(generator)*galaxy_size, z = pos_dis(generator)*galaxy_size;
            cout << setw(12) << mass << setw(12) << x << setw(12) << y << setw(12) << z << endl;
            body star(mass,x,y,z,0,0,0);
            galaxy.add(star);
        }
    }
...
*/
    ...
    **********************************************/
    if(method==1){
        start = clock();
        galaxy.RK4(output,steps,tmax);
        finish = clock();
    }
    else if(method==2){
        start = clock();
        galaxy.verlet(output,steps,tmax);
        finish = clock();
    }
