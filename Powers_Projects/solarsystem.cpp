...
// sets blank solar system
solarsystem::solarsystem()
{
}

// adds a planet class to current solar system
...
// changes data within a planet vector
...
// starts RK4 solver
void solarsystem::RK4solver(vector<planet> vec, double h, double tmax)
{

    mat y_i(num_planets,7);
    mat y_i_temp(num_planets,7);
    mat k1(num_planets,7);
    mat k2(num_planets,7);
    mat k3(num_planets,7);
    mat k4(num_planets,7);
    ...
    if (output.is_open())
    {
        ...
        while(t<tmax)
        {
            ...
            derivative(y_i, k1, num_planets);

            sum_matrix(y_i_temp, 1, y_i, 0.5*h, k1, num_planets);
            derivative(y_i_temp, k2, num_planets);

            sum_matrix( y_i_temp, 1,  y_i, 0.5*h,  k2, num_planets);

            derivative( y_i_temp,  k3, num_planets);

            sum_matrix( y_i_temp, 1,  y_i, h,  k3, num_planets);

            derivative( y_i_temp,  k4, num_planets);
...
// starts Verlet solver
void solarsystem::Verletsolver(vector<planet> vec, double h, double tmax){

    mat y_i(num_planets,7);
    mat r_i_dt(num_planets,7);
    mat a_dt(num_planets,7);
    mat v_dt(num_planets,7);
    mat v_dt_2(num_planets,7);
    ...
    if (output.is_open())
    {
        ...
        while(t<tmax){
            ...
            derivative(y_i, a_dt, num_planets);
...
//takes numerical derivative of __________________
